...
Day.addEventListener('click',(event)=>{
    ...
    if(event.target.className!=='disabled'){
        clearEvent();
        todoTitle.textContent = `오늘은 무슨일을 하실겁니까? ${year}.${mon}.${event.target.textContent}`;
...
reset.addEventListener('click',()=>{
    const result = prompt(`썼었던 내용들을 모두 지우시겠습니까? 지우시려면 y를 하지 않으시려면 n을 입력해 주십시오${year} ${mon} ${DayOfChoice}`);
    const YMD = year+'-'+mon+'-'+DayOfChoice;
    if(result==='y'){
...
allReset.addEventListener('click',()=>{
    const result = prompt(`모든 날짜에 기록은 삭제 하시겠습니까?`);
    if(result==='y'){
        localStorage.clear();
        displayToDoOnDays();
    }
